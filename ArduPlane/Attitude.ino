...
{
	//climb rate controlled by throttle, calculated from climb rate error - RMG
	g.channel_throttle.servo_out = g.throttle_cruise + g.pidTeThrottle.get_pid(climb_rate_error); 

}

static void calc_nav_pitch()
{
	//Speed is dictated by pitch and kept constant, so airspeed error entered into pitch PID - RMG
...
//Calculate desired roll/yaw angles (in medium freq loop)
...
static void set_servos(void)
{
    ...
    if(control_mode == MANUAL) {
        ...
        g.channel_throttle.radio_out    =  sqrt(g.channel_throttle.radio_in);
		...
		if(control_mode > STABILIZE)	//throttle is not constrained in manual mode - RMG
		{
