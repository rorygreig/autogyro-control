...
#include <DerivativeFilter.h>	//DerivativeFliter class (inherits from Filter class)
...
static int32_t prev_altitude;
static int32_t altitudes[7];

//Difference between current climb rate and target climb rate based on altitude error
static int32_t climb_rate_error;
...
//target climb rate based on altitude error
static float current_climb_rate;
static float target_climb_rate;
 ...
 *  }*/
    ...
    case 2:
        ...
        if (airspeed.enabled()) {	//Changed this so it happens in HIL attitude mode as well - RMG
            read_airspeed();		//airspeed error is calculated in here
        }
...
static void update_current_flight_mode(void)
{
	...
	case STABILIZE:
		nav_roll_cd        = 0;
		nav_pitch_cd       = 25;		//Target pitch - this should equal optimal autogyro climb pitch - RMG
...
{
#if HIL_MODE == HIL_MODE_ATTITUDE	
	prev_altitude = current_loc.alt;
    current_loc.alt = g_gps->altitude;

	current_climb_rate = calc_gps_climb(current_loc.alt);	//calc current climb rate, governs throttle for altitude control - RMG
...
//1st order noise robust differentiator function, differentiates GPS altitude to get climb rate - RMG
static float calc_gps_climb(int32_t new_alt)
{
	...
	slope = (5*(altitudes[4]-altitudes[2]) + 4*(altitudes[5]-altitudes[1]) + (altitudes[6]-altitudes[0]))/3.2;
